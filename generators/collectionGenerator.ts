 ...
 */
import { camelCase, toTitleCase, type GeneratorResult } from '../utils/index.js';
...
type AccessFunction = (args: AccessArgs) => AccessResult | Promise<AccessResult>;
...
interface ImageSize {
    ...
    generateImageName?: (args: { height: number; sizeName: string; extension: string; width: number }) => string;
}
...
interface UploadConfig {
    ...
    cacheTags?: boolean;
    handlers?: any[];
    externalFileHeaderFilter?: (headers: Record<string, any>) => Record<string, any>;
    pasteURL?: boolean | {
        allowList: Array<{
            hostname: string;
            pathname?: string;
 ...
 */
export async function generateCollection(options: CollectionGeneratorOptions): Promise<GeneratorResult> {
    const {
        slug,
...
const ${camelCase(slug)}: CollectionConfig = {
    ...
    code += generateAccessControl(access, typeof auth === 'boolean' ? auth : Boolean(auth), typeof versions === 'boolean' ? versions : Boolean(versions));
    ...
    if (hooks.length > 0) {
        code += ',\n  hooks: {';
        const hooksByTrigger = hooks.reduce((acc, hook) => {
            const { trigger } = hook;
 ...
 */
function generateAdminConfig(admin: AdminConfig = {}, fields: CollectionGeneratorOptions['fields'] = []): string {
    if (!admin) {
        admin = {};
    }

    const useAsTitle = admin.useAsTitle || (fields && fields.length > 0 ? fields[0]?.name : 'id');
    const defaultColumns = admin.defaultColumns || (fields ? fields.slice(0, 3).map(f => f.name) : ['id']);
    const {
 ...
 */
function generateAccessControl(access: CollectionAccess = {}, auth: boolean = false, versions: boolean = false): string {
    ...
    crudOperations.forEach(operation => {
        if (access[operation] !== undefined) {
            if (typeof access[operation] === 'boolean') {
                code += `    ${operation}: ${access[operation]},\n`;
            } else if (typeof access[operation] === 'function') {
                code += `    ${operation}: ${access[operation].toString()},\n`;
            }
        }
    });
...
function generateAuthConfig(auth: CollectionGeneratorOptions['auth']): string {
  ...
  auth: {
    ...
    if (maxLoginAttempts) authConfig += `\n    maxLoginAttempts: ${maxLoginAttempts},`;
    if (lockTime) authConfig += `\n    lockTime: ${lockTime},`;
    if (depth) authConfig += `\n    depth: ${depth},`;

    if (cookies) {
        authConfig += '\n    cookies: {';
        if (typeof cookies.secure !== 'undefined') authConfig += `\n      secure: ${cookies.secure},`;
        if (cookies.sameSite) authConfig += `\n      sameSite: '${cookies.sameSite}',`;
    ...
    if (typeof useAPIKey !== 'undefined') authConfig += `\n    useAPIKey: ${useAPIKey},`;
    if (disableLocalStrategy) authConfig += `\n    disableLocalStrategy: ${disableLocalStrategy},`;
    if (removeTokenFromResponses) authConfig += `\n    removeTokenFromResponses: ${removeTokenFromResponses},`;
...
function generateUploadConfig(upload: CollectionGeneratorOptions['upload']): string {
    if (!upload) return '';
  ...
  upload: {
    ...
    if (typeof focalPoint !== 'undefined') uploadConfig += `\n    focalPoint: ${focalPoint},`;
    if (formatOptions) uploadConfig += `\n    formatOptions: ${JSON.stringify(formatOptions)},`;
    if (resizeOptions) uploadConfig += `\n    resizeOptions: ${JSON.stringify(resizeOptions)},`;
    if (trimOptions) uploadConfig += `\n    trimOptions: ${JSON.stringify(trimOptions)},`;
    ...
    if (typeof cacheTags !== 'undefined') uploadConfig += `\n    cacheTags: ${cacheTags},`;
    ...
    if (typeof bulkUpload !== 'undefined') uploadConfig += `\n    bulkUpload: ${bulkUpload},`;
    if (typeof displayPreview !== 'undefined') uploadConfig += `\n    displayPreview: ${displayPreview},`;
